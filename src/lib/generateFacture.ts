...
function generateQRCodeSVG(text: string, size: number = 120): string {
  // QR code simplifié via api publique — encodé en data URI SVG
  const encoded = encodeURIComponent(text)
  return `https://api.qrserver.com/v1/create-qr-code/?size=${size}x${size}&data=${encoded}&bgcolor=ffffff&color=0F172A&qzone=1`
}
...
export function generateFactureHTML(data: FactureData): string {
  ...
  const qrUrl = generateQRCodeSVG(`PM:${data.candidatureId}:${factureNum}`)
...
<body>
  <div class="page">
    ...
    <div class="event-block">
      <div>
        <div class="event-item-label">Événement</div>
        <div class="event-item-value">${data.eventTitle}</div>
      </div>
      <div>
        <div class="event-item-label">Date du marché</div>
        <div class="event-item-value">${data.eventDate}</div>
      </div>
      <div>
        <div class="event-item-label">Lieu · Emplacement</div>
        <div class="event-item-value">${data.eventLocation}${data.emplacement ? ` · N° ${data.emplacement}` : ''}</div>
      </div>
    </div>
    ...
    <div class="table-section">
      ...
      <table>
        ...
        <tbody>
          <tr>
            <td>
              <div class="td-desc">Redevance d'occupation du domaine public (AOT)</div>
              <div class="td-sub">Autorisation d'installation sur voie publique · ${data.eventTitle}</div>
            </td>
    ...
    <div class="footer">
      ...
      <div class="qr-section">
        <img src="${qrUrl}" width="110" height="110" alt="QR Code" style="border-radius: 8px; border: 1px solid #E2E8F0;" />
        <div class="qr-label">Contrôle entrée</div>
  ...
  <div class="no-print" style="background: #F8FAFC; border-top: 1px solid #E2E8F0; padding: 16px; text-align: center; display: flex; gap: 12px; justify-content: center;">
    <button onclick="window.print()" style="background: #4F46E5; color: white; border: none; border-radius: 10px; padding: 12px 28px; font-size: 14px; font-weight: 600; cursor: pointer;">
      Imprimer / Enregistrer en PDF
    </button>
    <button onclick="window.close()" style="background: white; color: #64748B; border: 1px solid #E2E8F0; border-radius: 10px; padding: 12px 20px; font-size: 14px; font-weight: 500; cursor: pointer;">
      Fermer
    </button>
  </div>
</body>
...
export function openFacturePDF(data: FactureData) {

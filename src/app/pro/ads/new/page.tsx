      ...
      {steps.map((label, i) => {
        ...
        return (
          <div key={i} style={{ display: 'flex', alignItems: 'center', flex: i < 3 ? 1 : 'none' }}>
            ...
            {i < 3 && (
              <div style={{ flex: 1, height: 2, background: done ? '#10B981' : '#F3F4F6', margin: '0 4px 14px', transition: 'background 0.3s' }} />
            )}
          </div>
        )
      })}
...
function Step1({ data, onChange, onNext }: { data: AdData; onChange: (d: Partial<AdData>) => void; onNext: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
      <p style={{ fontFamily: '"Playfair Display", Georgia, serif', fontSize: 22, fontWeight: 700, color: '#111827', marginBottom: 6 }}>
        Votre photo
      </p>
      <p style={{ fontSize: 14, color: '#6B7280', marginBottom: 24, lineHeight: 1.6 }}>
        Choisissez une belle photo de votre stand ou de vos produits. Elle sera la première chose que les visiteurs verront.
      </p>
      ...
      <div
        ...
        style={{ border: `2px dashed ${data.photoPreview ? '#10B981' : '#E5E7EB'}`, borderRadius: 20, overflow: 'hidden', cursor: 'pointer', transition: 'all 0.2s', background: data.photoPreview ? 'transparent' : '#FAFAFA', minHeight: 220, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 24 }}
      >
        {data.photoPreview ? (
    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
      ...
      <div style={{ borderRadius: 18, overflow: 'hidden', background: '#111827', marginBottom: 24, display: 'flex', alignItems: 'stretch' }}>
        <div style={{ width: 80, flexShrink: 0 }}>
          {data.photoPreview
            ? <img src={data.photoPreview} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
            : <div style={{ width: '100%', height: '100%', background: '#1F2937', minHeight: 80 }} />
          }
        </div>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <button onClick={onNext} disabled={!valid}
          style={{ flex: 2, background: valid ? '#111827' : '#F3F4F6', color: valid ? 'white' : '#9CA3AF', border: 'none', borderRadius: 14, padding: '14px', fontSize: 14, fontWeight: 700, cursor: valid ? 'pointer' : 'not-allowed', fontFamily: '"DM Sans", sans-serif', transition: 'all 0.2s' }}>
          Continuer →
        </button>
      </div>
    </motion.div>
...
function Step3({ data, onChange, onNext, onBack }: { data: AdData; onChange: (d: Partial<AdData>) => void; onNext: () => void; onBack: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
      ...
      <p style={{ fontSize: 14, color: '#6B7280', marginBottom: 24, lineHeight: 1.6 }}>Votre pub sera visible par tous les visiteurs de ce marché dans l'app Whatmarket.</p>

      {loading ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {[1, 2, 3].map(i => <div key={i} style={{ height: 72, borderRadius: 14, background: '#F3F4F6', animation: 'shimmer 1.4s ease infinite' }} />)}
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 24 }}>
          ...
          {events.length === 0 && (
            <div style={{ textAlign: 'center', padding: '32px 0', color: '#9CA3AF', fontSize: 13 }}>Aucun marché disponible pour le moment</div>
          )}
        </div>
      )}
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <button onClick={onBack} style={{ flex: 1, background: '#F9FAFB', color: '#374151', border: '1px solid #E5E7EB', borderRadius: 14, padding: '14px', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: '"DM Sans", sans-serif' }}>← Retour</button>
        <button onClick={onNext} disabled={!data.eventId}
          style={{ flex: 2, background: data.eventId ? '#111827' : '#F3F4F6', color: data.eventId ? 'white' : '#9CA3AF', border: 'none', borderRadius: 14, padding: '14px', fontSize: 14, fontWeight: 700, cursor: data.eventId ? 'pointer' : 'not-allowed', fontFamily: '"DM Sans", sans-serif', transition: 'all 0.2s' }}>
          Continuer →
        </button>
      </div>
    </motion.div>
...
function Step4({ data, onBack }: { data: AdData; onBack: () => void }) {
  ...
  if (paid) return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} style={{ textAlign: 'center', padding: '20px 0' }}>
      <div style={{ width: 72, height: 72, background: '#F0FDF4', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px', border: '2px solid #BBF7D0' }}>
      ...
      <p style={{ fontFamily: '"Playfair Display", Georgia, serif', fontSize: 24, fontWeight: 700, color: '#111827', marginBottom: 8 }}>Votre pub est en ligne ! 🚀</p>
      <p style={{ fontSize: 14, color: '#6B7280', lineHeight: 1.7, marginBottom: 24 }}>
        Votre pub pour <strong>{data.nom}</strong> est maintenant visible dans Whatmarket pour le <strong>{data.eventTitle}</strong>.<br />
        Un email de confirmation a été envoyé à <strong>{email}</strong>.
      </p>
      <div style={{ background: '#F9F8F6', border: '1px solid #E5E7EB', borderRadius: 16, padding: '16px 20px', textAlign: 'left', marginBottom: 24 }}>
        <p style={{ fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Récapitulatif</p>
        {[
          ['Commerce', data.nom],
          ['Offre', data.offre],
          ['Marché', data.eventTitle],
          ['Montant', '20,00 €'],
        ].map(([l, v], i) => (
          <div key={i} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, paddingBottom: i < 3 ? 8 : 0, marginBottom: i < 3 ? 8 : 0, borderBottom: i < 3 ? '1px solid #F3F4F6' : 'none' }}>
            <span style={{ color: '#9CA3AF' }}>{l}</span>
            <span style={{ fontWeight: 600, color: '#111827' }}>{v}</span>
          </div>
        ))}
      </div>
      <a href="/whatmarket" style={{ display: 'block', background: '#111827', color: 'white', textDecoration: 'none', borderRadius: 14, padding: '15px', fontSize: 14, fontWeight: 700, textAlign: 'center', fontFamily: '"DM Sans", sans-serif' }}>
    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
        {[
          ...
          { icon: '⚡', text: 'Publication immédiate' },
          { icon: '📧', text: 'Confirmation par email' },
        ].map((t, i) => (
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <button onClick={handlePay} disabled={!email || paying}
          ...
          {paying
            ? <><div style={{ width: 15, height: 15, border: '2px solid rgba(255,255,255,0.4)', borderTopColor: 'white', borderRadius: '50%', animation: 'spin 0.7s linear infinite' }} /> Traitement...</>
            : '🔒 Payer 20€ et Publier'
          }
        </button>
      </div>
...
export default function BoostMyBusiness() {
  ...
  const [adData, setAdData] = useState<AdData>({
    photo: null, photoPreview: '', nom: '', offre: '', detail: '', eventId: '', eventTitle: ''
  })
    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#F9F8F6', fontFamily: '"DM Sans", system-ui, sans-serif' }}>
        ...
        <div style={{ background: 'white', borderBottom: '0.5px solid #E5E7EB', padding: '16px 24px', display: 'flex', alignItems: 'center', gap: 12, position: 'sticky', top: 0, zIndex: 10 }}>
          ...
          <div style={{ marginLeft: 'auto', background: '#FEF3C7', color: '#92400E', fontSize: 11, fontWeight: 700, padding: '3px 10px', borderRadius: 100 }}>
            🚀 Pro
          </div>
        </div>
        ...
        <div style={{ maxWidth: 480, margin: '0 auto', padding: '32px 20px 60px' }}>
          ...
          {step === 1 && (
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: 32, textAlign: 'center' }}>
              <p style={{ fontFamily: '"Playfair Display", Georgia, serif', fontSize: 28, fontWeight: 900, color: '#111827', lineHeight: 1.2, marginBottom: 8 }}>
                Soyez en tête d'affiche
              </p>
              <p style={{ fontSize: 14, color: '#6B7280', lineHeight: 1.7 }}>
                Touchez des milliers de visiteurs du marché. Votre pub, leur téléphone, en temps réel.
              </p>
            </motion.div>
          )}
          ...
          {step === 1 && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}
              style={{ marginTop: 24, display: 'flex', gap: 16, justifyContent: 'center' }}>
              {[
                { value: '2 400', label: 'visiteurs/marché' },
                { value: '68%', label: 'taux de clic' },
                { value: '20€', label: 'seulement' },
              ].map((s, i) => (

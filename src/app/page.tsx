...
function Calculator() {
  ...
  return (
    <div style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 12 }} className="p-6">
      <p className="text-xs font-semibold uppercase tracking-widest mb-5" style={{ color: '#64748B' }}>Simulateur de coût d'emplacement</p>
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div>
          <label className="text-xs text-slate-500 mb-1 block">Largeur stand (m)</label>
          <input type="range" min={1} max={10} value={width} onChange={e => setWidth(+e.target.value)} className="w-full accent-indigo-600" />
        ...
        <div>
          ...
          <input type="range" min={1} max={12} value={length} onChange={e => setLength(+e.target.value)} className="w-full accent-indigo-600" />
      ...
      <div className="mb-5">
        ...
        <div className="flex gap-2">
          {[{ val: 'none', label: 'Aucun' }, { val: 'mono', label: 'Monophasé' }, { val: 'tri', label: 'Triphasé' }].map(opt => (
            <button key={opt.val} onClick={() => setElectric(opt.val as any)}
              style={{ border: electric === opt.val ? '1.5px solid #4F46E5' : '1px solid #E2E8F0', background: electric === opt.val ? '#EEF2FF' : 'white', color: electric === opt.val ? '#4F46E5' : '#64748B', borderRadius: 8 }}
      ...
      <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 10 }} className="p-4 space-y-2">
        <div className="flex justify-between text-sm"><span className="text-slate-500">Surface ({area} m² × {BASE_RATE}€/m²)</span><span className="font-medium text-slate-900">{locationCost.toFixed(2)} €</span></div>
        <div className="flex justify-between text-sm"><span className="text-slate-500">Redevance AOT (1.20€/m²)</span><span className="font-medium text-slate-900">{aotFee.toFixed(2)} €</span></div>
        {electricCost > 0 && <div className="flex justify-between text-sm"><span className="text-slate-500">Branchement électrique</span><span className="font-medium text-slate-900">{electricCost.toFixed(2)} €</span></div>}
      ...
      <p className="text-xs text-slate-400 mt-3">* Simulation indicative. Le tarif final est fixé par l'organisateur.</p>
    </div>
  )
}
    ...
    <div style={{ background: '#F8FAFC', color: '#0F172A', fontFamily: 'system-ui, sans-serif' }} className="min-h-screen">
      ...
      <motion.header initial={{ y: -20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.4 }}
        ...
        <div className="max-w-6xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            {/* ✅ Lien espace organisateur dans la nav */}
            <button onClick={() => router.push('/auth/mairie')}
              style={{ fontSize: 13, color: '#4F46E5', fontWeight: 600, background: '#EEF2FF', padding: '6px 12px', borderRadius: 8, border: '1px solid #C7D2FE' }}
              className="hidden md:flex items-center gap-1.5 hover:opacity-90 transition-opacity">
              <Building2 size={13} /> Espace organisateur
            </button>
        ...
        <div className="max-w-5xl mx-auto">
          <AnimatedSection>
            ...
            <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-3">
              ...
              {/* ✅ Bouton organisateur → /auth/mairie */}
              <button onClick={() => router.push('/auth/mairie')}
      ...
      <section id="workflow" className="py-20 px-6" style={{ borderBottom: '1px solid #E2E8F0', background: 'white' }}>
        <div className="max-w-6xl mx-auto">
          <AnimatedSection>
            <motion.p variants={fadeUp} style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.12em', color: '#94A3B8', textTransform: 'uppercase' }} className="mb-3">Architecture du processus</motion.p>
            <motion.h2 variants={fadeUp} style={{ fontSize: 32, fontWeight: 700, letterSpacing: '-0.02em' }} className="mb-4">Du dossier papier à la validation numérique</motion.h2>
          ...
          <AnimatedSection className="grid md:grid-cols-4 gap-4">
            {[
              { step: '01', icon: <FileCheck size={18} style={{ color: '#4F46E5' }} />, title: 'Inscription & Upload', desc: "L'exposant dépose son extrait Kbis certifié et son certificat d'assurance RC Pro. Les fichiers sont chiffrés AES-256 et stockés dans un bucket privé." },
              { step: '02', icon: <ShieldCheck size={18} style={{ color: '#4F46E5' }} />, title: 'Certification OCR', desc: "Notre moteur IA extrait le numéro SIREN, la raison sociale et la date de création. Une requête temps réel vers l'API SIRENE de l'INSEE confirme le statut actif." },
              { step: '03', icon: <CheckCircle size={18} style={{ color: '#4F46E5' }} />, title: 'Validation Placier', desc: "L'agent placier consulte le dossier certifié sur son dashboard. Il accepte ou refuse en 1 clic. La décision génère automatiquement un courrier de notification." },
              { step: '04', icon: <Lock size={18} style={{ color: '#4F46E5' }} />, title: 'AOT & Paiement', desc: "L'Autorisation d'Occupation du Domaine Public (AOT) est générée avec les coordonnées GPS de l'emplacement. Le paiement sécurisé est traité via Stripe." },
            ].map((item, i) => (
      ...
      <section id="admin" className="py-20 px-6" style={{ borderBottom: '1px solid #E2E8F0' }}>
        <div className="max-w-6xl mx-auto">
          <AnimatedSection>
            ...
            <motion.h2 variants={fadeUp} style={{ fontSize: 32, fontWeight: 700, letterSpacing: '-0.02em', maxWidth: 560 }} className="mb-12">Sécurité juridique et gain de temps pour l'agent placier</motion.h2>
          </AnimatedSection>
          <AnimatedSection className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <motion.div variants={fadeUp} className="md:col-span-2" style={{ background: '#0F172A', borderRadius: 12, padding: 32, minHeight: 280 }}>
              ...
              <h3 style={{ color: 'white', fontWeight: 600, fontSize: 18, marginBottom: 12 }}>Génération automatique des arrêtés municipaux</h3>
              <p style={{ color: '#94A3B8', fontSize: 14, lineHeight: 1.7, maxWidth: 420 }}>À chaque validation de candidature, PulseMarket génère un projet d'arrêté municipal pré-rempli avec coordonnées, SIREN vérifié, surface d'occupation, GPS et montant AOT calculé automatiquement.</p>
            ...
            <motion.div variants={fadeUp} style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 12, padding: 24 }}>
              ...
              <p style={{ fontSize: 13, color: '#64748B', lineHeight: 1.6 }}>de temps administratif par événement. Plus d'appels téléphoniques, de relances email ni de dossiers papier manquants.</p>
            </motion.div>
            <motion.div variants={fadeUp} style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 12, padding: 24 }}>
              ...
              <p style={{ fontSize: 13, color: '#64748B', lineHeight: 1.6 }}>Chaque dossier est archivé avec horodatage légal. Les certificats d'assurance RC Pro sont vérifiés à date de validité. Votre responsabilité est couverte.</p>
            </motion.div>
            <motion.div variants={fadeUp} className="md:col-span-2" style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 12, padding: 24 }}>
              ...
              <p style={{ fontSize: 13, color: '#64748B', lineHeight: 1.6, maxWidth: 480 }}>Envoyez en un clic un message à l'ensemble des exposants validés. Alertes météo, changement de placement, report de date — tous notifiés instantanément.</p>
            </motion.div>
          </AnimatedSection>

          {/* ✅ CTA organisateur dans la section admin */}
          <AnimatedSection className="mt-8">
            <motion.div variants={fadeUp} style={{ background: 'linear-gradient(135deg, #0F172A, #1E293B)', borderRadius: 14, padding: '24px 32px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 16 }}>
              <Building2 size={24} style={{ color: '#818CF8' }} />
              <div>
                <p style={{ fontSize: 18, fontWeight: 700, color: 'white', marginBottom: 6 }}>Vous êtes une mairie, un comité des fêtes ou une association ?</p>
                <p style={{ fontSize: 14, color: '#64748B' }}>Créez votre espace organisateur en 2 minutes — gratuit pour démarrer</p>
              </div>
              <button onClick={() => router.push('/auth/mairie')}
                style={{ background: '#4F46E5', color: 'white', padding: '12px 28px', borderRadius: 10, fontWeight: 600, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8 }}
      ...
      <section id="exposant" className="py-20 px-6" style={{ borderBottom: '1px solid #E2E8F0', background: 'white' }}>
        <div className="max-w-6xl mx-auto">
          <AnimatedSection>
            ...
            <motion.h2 variants={fadeUp} style={{ fontSize: 32, fontWeight: 700, letterSpacing: '-0.02em', maxWidth: 560 }} className="mb-12">Votre portefeuille numérique d'exposant</motion.h2>
          </AnimatedSection>
          <div className="grid md:grid-cols-2 gap-8 items-start">
            <AnimatedSection className="space-y-4">
              {[
                { icon: <FileCheck size={16} style={{ color: '#4F46E5' }} />, title: 'Extraits Kbis et RC Pro centralisés', desc: "Déposez vos documents légaux une seule fois. Notre système OCR extrait automatiquement les informations clés et les associe à chaque candidature." },
                { icon: <Zap size={16} style={{ color: '#4F46E5' }} />, title: 'Candidature en 1 clic avec dossier complet', desc: "Lorsqu'un événement vous correspond, postulez instantanément. Votre dossier certifié est transmis automatiquement à l'organisateur." },
                { icon: <MapPin size={16} style={{ color: '#4F46E5' }} />, title: 'Alertes géolocalisées sur les opportunités', desc: "Définissez votre zone de chalandise et recevez une notification dès qu'un organisateur publie un événement dans votre secteur." },
                { icon: <Lock size={16} style={{ color: '#4F46E5' }} />, title: "Badge 'Dossier Vérifié' — gage de confiance", desc: "Après vérification de votre SIREN via l'API SIRENE et validation de vos documents, un badge officiel est apposé sur votre profil." },
              ].map((item, i) => (
                <motion.div key={i} variants={fadeUp} style={{ background: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: 12, padding: 20, display: 'flex', gap: 16 }}>
                  <div style={{ background: '#EEF2FF', borderRadius: 8, width: 32, height: 32, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{item.icon}</div>
                  <div>
            ...
            <AnimatedSection>
              <motion.div variants={fadeUp}><Calculator /></motion.div>
            </AnimatedSection>
          </div>
        </div>
      </section>
      ...
      <section id="tarifs" className="py-20 px-6" style={{ borderBottom: '1px solid #E2E8F0' }}>
        <div className="max-w-4xl mx-auto">
          <AnimatedSection>
            ...
            <motion.h2 variants={fadeUp} style={{ fontSize: 32, fontWeight: 700, textAlign: 'center', letterSpacing: '-0.02em', marginBottom: 8 }}>Modèle SaaS transparent</motion.h2>
            <motion.p variants={fadeUp} style={{ color: '#64748B', textAlign: 'center', marginBottom: 48, fontSize: 15 }}>Les administrations démarrent gratuitement. La valeur créée justifie l'abonnement exposant.</motion.p>
          </AnimatedSection>
          <AnimatedSection className="grid md:grid-cols-3 gap-6">
            {[
              { name: 'Exposant Gratuit', price: '0€', period: 'pour toujours', features: ['Consultation carte des événements', 'Création profil exposant', 'Upload Kbis & RC Pro', '1 candidature / mois', 'Vérification SIREN basique'], cta: 'Commencer', accent: false, path: '/auth?tab=signup' },
              { name: 'Exposant Pro', price: '20€', period: 'par mois', features: ['Candidatures illimitées', "Badge Dossier Vérifié (API SIRENE)", 'Alertes SMS & Push géolocalisées', 'Accès événements exclusifs', 'Archivage dossier 3 ans'], cta: 'Passer en Pro', accent: true, path: '/auth?tab=signup' },
              { name: 'Administration', price: '150€', period: 'par mois', features: ['Événements illimités', 'Génération arrêtés municipaux', 'Messagerie groupée exposants', 'Export comptable CSV/PDF', 'Support dédié agent placier'], cta: 'Créer mon espace', accent: false, path: '/auth/mairie' },
            ].map((plan, i) => (
              <motion.div key={i} variants={fadeUp}
                ...
                {plan.accent && <div style={{ position: 'absolute', top: -12, left: '50%', transform: 'translateX(-50%)', background: '#4F46E5', color: 'white', fontSize: 11, fontWeight: 700, padding: '3px 12px', borderRadius: 100 }}>RECOMMANDÉ</div>}
                ...
                {/* ✅ Plan Administration → /auth/mairie */}
                <button onClick={() => router.push(plan.path)}
                  style={{ width: '100%', background: plan.accent ? '#4F46E5' : 'transparent', color: plan.accent ? 'white' : '#0F172A', border: plan.accent ? 'none' : '1px solid #E2E8F0', padding: '10px 0', borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}
                  className="hover:opacity-90 transition-opacity">
                  {plan.cta}
                </button>
              </motion.div>
      ...
      <section className="py-24 px-6">
        <div className="max-w-2xl mx-auto text-center">
          <AnimatedSection>
            <motion.h2 variants={fadeUp} style={{ fontSize: 36, fontWeight: 700, letterSpacing: '-0.02em', marginBottom: 16 }}>Prêt à numériser vos marchés ?</motion.h2>
            ...
            <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-3 justify-center">
              <button onClick={() => router.push('/auth?tab=signup')}
                ...
                <Store size={16} /> Créer mon compte exposant <ArrowRight size={16} />
              </button>
              <button onClick={() => router.push('/auth/mairie')}
                ...
                className="hover:border-slate-400 transition-colors inline-flex items-center gap-2">
                <Building2 size={16} /> Espace organisateur
              </button>
            </motion.div>
          </AnimatedSection>
        </div>

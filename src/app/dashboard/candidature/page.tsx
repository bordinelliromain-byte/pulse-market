                  ...
                  <div style={{ padding: '16px 18px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2, 1fr)' : 'repeat(4, 1fr)', gap: 8, marginBottom: 16 }}>
                      {STAND_OPTIONS.map((opt) => (
                        <button key={opt.size} onClick={() => setSelectedSize(opt.size)}
                          style={{ position: 'relative', padding: '14px 10px', border: selectedSize === opt.size ? '2px solid #4F46E5' : '1px solid #E2E8F0', borderRadius: 10, background: selectedSize === opt.size ? '#EEF2FF' : 'white', cursor: 'pointer', textAlign: 'center', transition: 'all 0.2s' }}>
                          {opt.popular && <span style={{ position: 'absolute', top: -8, left: '50%', transform: 'translateX(-50%)', background: '#4F46E5', color: 'white', fontSize: 9, fontWeight: 700, padding: '2px 8px', borderRadius: 100, whiteSpace: 'nowrap' }}>POPULAIRE</span>}
                          <p style={{ fontSize: 16, fontWeight: 700, color: selectedSize === opt.size ? '#4F46E5' : '#0F172A', marginBottom: 2 }}>{opt.label}</p>
                          <p style={{ fontSize: 10, color: '#94A3B8', marginBottom: 6 }}>{opt.desc}</p>
                          <p style={{ fontSize: 13, fontWeight: 700, color: selectedSize === opt.size ? '#4F46E5' : '#0F172A' }}>{opt.price} €</p>
                          {selectedSize === opt.size && <div style={{ position: 'absolute', top: 6, right: 6 }}><CheckCircle size={13} style={{ color: '#4F46E5' }} /></div>}
                        </button>
                      ))}
                    </div>
                    ...
                    <div style={{ border: '1px solid #E2E8F0', borderRadius: 10, padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                      ...
                      <button onClick={() => setNeedsElectricity(!needsElectricity)}
                        style={{ width: 44, height: 24, borderRadius: 100, border: 'none', cursor: 'pointer', background: needsElectricity ? '#4F46E5' : '#E2E8F0', position: 'relative', transition: 'background 0.2s', flexShrink: 0 }}>
                      ...
                      <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 14px', border: `1px solid ${doc.ok ? '#BBF7D0' : '#FED7AA'}`, borderRadius: 10, background: doc.ok ? '#F0FDF4' : '#FFF7ED' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <div>
                            ...
                            <p style={{ fontSize: 11, color: doc.ok ? '#16A34A' : '#EA580C', marginTop: 1 }}>{doc.ok ? 'Vérifié ✓' : 'Manquant'}</p>
                          </div>
                        </div>
                        {!doc.ok && (
                ...
                <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 12, overflow: 'hidden' }}>
                  ...
                  <div style={{ padding: '16px 18px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#EEF2FF', border: '1px solid #C7D2FE', borderRadius: 10, padding: '12px 14px' }}>
                      <Shield size={15} style={{ color: '#4F46E5', flexShrink: 0 }} />
                      <div>
                    ...
                    <div style={{ display: 'flex', gap: 10 }}>
                      ...
                      <button onClick={handleSubmit} disabled={submitting}
                        style={{ flex: 2, background: submitting ? '#818CF8' : '#4F46E5', color: 'white', border: 'none', borderRadius: 10, padding: '12px 0', fontSize: 14, fontWeight: 600, cursor: submitting ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
              ...
              <div style={{ padding: '16px 18px' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: 12 }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, borderTop: '1px solid #E2E8F0', paddingTop: 8, marginTop: 2 }}>
                    ...
                    <span style={{ fontWeight: 800, color: '#4F46E5', fontSize: 16 }}>{totalTTC.toFixed(2)} €</span>
                  </div>
                </div>
                ...
                <div style={{ textAlign: 'center', fontSize: 11, color: '#94A3B8' }}>
                  ...
                  <div style={{ display: 'flex', gap: 2, justifyContent: 'center' }}>
                    {[...Array(5)].map((_, i) => <Star key={i} size={10} style={{ color: '#FBBF24' }} />)}
        ...
        {isMobile && step !== 3 && (
          <div style={{ background: 'white', border: '1px solid #E2E8F0', borderRadius: 12, padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <p style={{ fontSize: 18, fontWeight: 800, color: '#4F46E5' }}>{totalTTC.toFixed(2)} €</p>
            </div>

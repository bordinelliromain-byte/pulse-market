  ...
  useEffect(() => {
    const getData = async () => {
      ...
      const { data: apps } = await supabase.from('applications')
        .select(`*, events:event_id(id, title, start_date, end_date, location_name, image_url, price_per_spot)`)
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', background: '#F8FAFC', fontFamily: "'Inter', system-ui, sans-serif" }}>
      ...
      <div style={{ marginLeft: isMobile ? 0 : 220, flex: 1, minWidth: 0 }}>
        <header style={{ background: 'white', borderBottom: '1px solid #E2E8F0', padding: isMobile ? '0 14px 0 60px' : '0 28px', height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <div style={{ width: 28, height: 28, background: '#EEF2FF', borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              <CalendarCheck size={13} style={{ color: '#4F46E5' }} />
            </div>
            <div>
          ...
          <button onClick={() => router.push('/dashboard/evenements')}
            style={{ display: 'flex', alignItems: 'center', gap: 6, background: '#4F46E5', color: 'white', border: 'none', borderRadius: 8, padding: '7px 12px', fontSize: 12, fontWeight: 600, cursor: 'pointer', flexShrink: 0 }}>
        ...
        <main style={{ padding: isMobile ? '14px' : '24px 32px', display: 'flex', flexDirection: 'column', gap: 24 }}>
                  ...
                  {upcoming.map((c) => {
                    ...
                    return (
                      <motion.div key={c.id} variants={fadeUp}
                        style={{ background: 'white', border: `1px solid ${isToday ? '#4F46E5' : isSoon ? '#C7D2FE' : '#E2E8F0'}`, borderRadius: 14, overflow: 'hidden', display: 'flex' }}>
                        ...
                        <div style={{ flex: 1, padding: isMobile ? '14px' : '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 6, flexWrap: 'wrap' }}>
                              ...
                              {isSoon && !isToday && <span style={{ background: '#EEF2FF', color: '#4F46E5', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 100, flexShrink: 0 }}>BIENTÔT</span>}
                            </div>
                            <div style={{ display: 'flex', gap: isMobile ? 10 : 16, fontSize: 11, color: '#64748B', flexWrap: 'wrap' }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                <Calendar size={10} style={{ color: '#4F46E5' }} />
                          ...
                          <div style={{ textAlign: 'center', flexShrink: 0 }}>
                            <div style={{ width: isMobile ? 52 : 64, height: isMobile ? 52 : 64, borderRadius: 12, background: isToday ? '#DC2626' : isSoon ? '#4F46E5' : '#F8FAFC', border: `1px solid ${isToday ? '#DC2626' : isSoon ? '#4F46E5' : '#E2E8F0'}`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
